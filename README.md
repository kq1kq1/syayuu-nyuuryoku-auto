...
### 4. Excelテンプレートと写真
- `社有入力テンプレート.xlsx` … 入力データ。**配布zipに同梱して渡す**（アプリ内に作成ボタンは無い / 下記C参照）。
- `物件写真/` … 写真フォルダ（`同仕様モデルハウス/`, `ホームズ写真/` など）。

### 5. 起動URL（urls.txt）
Chrome起動時に開くURLは `urls.txt` に書いてあります。**各サイトのログイン画面のURLだけ**なので
秘密情報を含まず、リポジトリと配布zipの両方にそのまま入っています。**受け取った人の設定作業はありません。**

ログインと物件の検索は、開いたChromeで各自が手で行います。ツールが担当するのは自動入力だけです。

> ⚠️ SUUMOのトークン付きURL（`?path=...`）は使っていません。トークンが発行できない前提の運用です。
> 発行できるようになったら `urls.txt` のSUUMOの行をトークン付きURLに差し替え、
> **`.gitignore` に `urls.txt` を戻して**ください（`.gitignore` にその旨のコメントを残してあります）。
...
1. **Chromeを起動**（デバッグポート9222 + 各サイトを自動で開く）
   ...
   - ⚠️ 初回はここで各サイトに手動ログイン
2. サイトの物件検索から目的の物件を探し、**その物件の編集ページ**を開いたままにする（ここまで手作業）
3. **アプリを起動**
   - `2.次にダブルクリック(自動入力アプリが起動する).bat`
4. 起動時ダイアログで **建築確認番号** と **物件価格** を入力
5. **物件タイプ**（新築 / 中古 / 土地）を選択
...
| 写真 / 写真（中古） | 順番・ファイル名・キャプション・文言・スーモスロット参考・**キャプション（土地）** |
...
| ピタクラ / ピタクラ（中古） | ピタクラ用（新築側に **キャプション（土地）** 列あり） |
...
├── add_tochi_columns.py       # 既存テンプレートに土地キャプション列を追加
├── automation/
...
├── urls.txt                   # 起動時に開くログイン画面URL（秘密情報なし・追跡対象）
├── requirements.txt
...
| Chromeは開くがサイトが出ない | `urls.txt` が欠けている | zipをフォルダごと展開し直す |
